...
@implementation CNLiveKVStorageItem
@end

@implementation CNLiveKVStorage{
...
- (BOOL)_dbInitialize {
    //pragma journal_mode = wal; pragma synchronous = normal; 不开启wal功能
    NSString *sql = @"create table if not exists manifest (key text, filename text, size integer, inline_data blob, modification_time integer, last_access_time integer, extended_data blob, primary key(key)); create index if not exists last_access_time_idx on manifest(last_access_time);";
...
- (CNLiveKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    CNLiveKVStorageItem *item = [CNLiveKVStorageItem new];
...
- (CNLiveKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    CNLiveKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            CNLiveKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CNLiveKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:CNLiveKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(CNLiveKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"CNLiveKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > CNLiveKVStorageTypeMixed) {
        NSLog(@"CNLiveKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _trashQueue = dispatch_queue_create("com.CNLive.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
            NSLog(@"CNLiveKVStorage init error:%@", error);
            return nil;
        }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"CNLiveKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == CNLiveKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != CNLiveKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case CNLiveKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case CNLiveKVStorageTypeFile:
        case CNLiveKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (CNLiveKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (CNLiveKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CNLiveKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (CNLiveKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CNLiveKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != CNLiveKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            CNLiveKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
